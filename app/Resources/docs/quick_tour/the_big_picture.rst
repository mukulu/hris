...
Start using Hris in 10 minutes! This chapter will walk you through some
of the most important concepts behind Hris and explain how you can get
started quickly by showing you a simple project in action.

If you've used a web framework before, you should feel right at home with
Hris. If not, welcome to a whole new way of developing web applications!

Hris is based on Symfony2 Web framework, built as an extension of Symfony2
standard package, thus Symfony2 and Hris will be used interchangeably throughout.
...
.. tip::
    ...
    you'll extract Hris into so that the eventual URL to your application
    is "http://localhost/web/app_dev.php". You can also extract Hris
    first and then start the web server in the Hris "web" directory. If
    you do this, the URL to your application will be "http://localhost/app_dev.php".

Ready? Start by downloading the "`Human Resource Information system`_", a development
:term:`distribution` that is preconfigured with all dependencies pre configured.

After unpacking the archive under your web server root directory, you should
have a ``hris-master/`` directory, rename it to ``hris/`` so that looks like this:

.. code-block:: text

    www/ <- your web root directory
        hris/ <- the unpacked archive
            app/
                cache/
                config/
                logs/
...
.. note::
    ...
    .. code-block:: bash
        ...
        $ composer.phar create-project hrisproject/hris path/to/install dev-master

        # remove the Git history
        $ rm -rf .git

    For an exact version, replace "dev-master" with the latest Hris version
    (e.g. 3.0). For details, see the `Hris Installation Page`_
...
Hris comes with a visual server configuration tester to help avoid some
...
.. note::
    ...
    All of the example URLs assume that you've downloaded and unzipped hris
    directly into the web server web root and renamed it to hris. If you've followed the directions
    above and unzipped the `hris-master` directory into your web root and renamed to `hris`, then add
    `/hris/web` after `localhost` for all the URLs you see:

    .. code-block:: text

        http://localhost/hris/web/config.php

.. note::

    All of the example URLs assume that you've downloaded and unzipped ``Hris``
    directly into the web server web root. If you've followed the directions
    above and done this, then add ``/hris/web`` after ``localhost`` for all
    the URLs you see:
    ...
    webserver or virtual host to the ``hris/web/`` directory. In that
...
your first Hris2 login page:

.. code-block:: text

    http://localhost/app_dev.php/

HRIS will direct you to login screen ready for usage of hris
...
HRIS has been re-developed from the scratch to use Symfony framework.
...
logic in the same script. To achieve this goal in Hris through Symfony, you'll first
...
To understand more how Symfony2 work we'll examine login page to hris.
Go to the following URL to access login page (replace *Fabien* with your first name):

.. code-block:: text

    http://localhost/app_dev.php/login
...
.. _Human Resource Information system:      http://github.com/hrisproject/hris/archive/master.zip
...
.. _`Hris Installation Page`:    http://github.com/hrisproject/hris/blob/master/README.md
